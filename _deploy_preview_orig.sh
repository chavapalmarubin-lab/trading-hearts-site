#!/bin/bash
# Deploy the ORIGINAL site (with simplified nav) to the Cloudflare Pages PREVIEW branch. Production (main) untouched.
exec > /tmp/th_prevorig.log 2>&1
...
rsync -a --delete --exclude='index.html.bak_*' --exclude='_deploy*.sh' --exclude='.git' --exclude='.wrangler' --exclude='media/_orig_png' --exclude='qa' --exclude='index_experience.html' "$SITE"/ "$STAGE"/
